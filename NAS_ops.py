...
def process_bam_per_individual(bam_files, global_exon_junctions_file, PTC_exon_junctions_file, out_folder, PTC_file, syn_nonsyn_file, out_prefix, exon_junctions_bam_output_folder, kw_dict):
    ...
    if len(bam_files) > 0:
        ...
        if "phase" in kw_dict:
            ...
            phase = False
        if "mapq_intervals" in kw_dict:
            mapq_intervals = kw_dict['mapq_intervals']
        else:
            mapq_intervals = False
        if "exclude_xt_filter" in kw_dict:
            exclude_xt_filter = kw_dict['exclude_xt_filter']
        else:
            exclude_xt_filter = False
        if "nm_format" in kw_dict:
            nm_format = kw_dict['nm_format']
        else:
            nm_format = False
        ...
        for pos, bam_file in enumerate(bam_files):
            ...
            if not os.path.isfile(output_file):
                ...
                read_count_file_name = "{0}/{1}_read_count_sample_name.txt".format(exon_junctions_bam_output_folder, sample_name)
                read_count = None
                ...
                if not exclude_xt_filter:
                    ##filter bam by xt tag XT=U
                    bmo.bam_xt_filter(mapq_flag_filtered_bam, mapq_flag_xt_filtered_bam, xt_filter="U")
                else:
                    gen.run_process(["cp", mapq_flag_filtered_bam, mapq_flag_xt_filtered_bam])

                ##filter bam by nm tag NM<=6
                bmo.bam_nm_filter(mapq_flag_xt_filtered_bam, mapq_flag_xt_nm_filtered_bam, nm_less_equal_to=6, nm_format = nm_format)
...
def ptc_snp_simulation(out_prefix, simulation_output_folder, ptc_file, syn_nonsyn_file, exon_junctions_file, bam_files, required_simulations, exon_junctions_bam_output_folder, use_old_sims = False, match_distance = None):
    ...
    #setup up simulation bam analysis output folder
    if match_distance:
        simulation_bam_analysis_output_folder = "{0}__analysis_simulation_ptc_snps_bam_analysis_match_distance".format(out_prefix)
    else:
