...
def main():
    ...
    args = gen.parse_arguments(description,  ["output_dir", "mutations_dir", "results_prefix", "process_mutations", "intersect_mutations", "convert_intersect", "get_relative_positions", "get_snp_status"], flags = [3,4,5,6,7], ints = [])
    output_dir, mutations_dir, results_prefix, process_mutations, intersect_mutations, convert_intersect, get_relative_positions, get_snp_status = args.output_dir, args.mutations_dir, args.results_prefix, args.process_mutations, args.intersect_mutations, args.convert_intersect, args.get_relative_positions, args.get_snp_status
    ...
    if convert_intersect or not os.path.isfile(bed_intersect_file):
    ...
    if get_relative_positions or not os.path.isfile(disease_snps_relative_exon_positions) or not os.path.isfile(disease_snps_relative_cds_positions):
        ...
        so.get_snp_relative_exon_position(bed_intersect_file, disease_snps_relative_exon_positions)
