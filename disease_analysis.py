...
def main():
    ...
    args = gen.parse_arguments(description,  ["output_dir", "mutations_dir", "rna_dir", "results_prefix", "subset", "subset_no", "clean_run", "snp_ops", "process_mutations", "intersect_mutations", "convert_intersect", "get_relative_positions", "get_snp_status", "process_reads"], flags = [4,6,7,8,9,10,11,12,13], ints = [5])
    output_dir, mutations_dir, rna_dir, results_prefix, subset, subset_no, clean_run, snp_ops, process_mutations, intersect_mutations, convert_intersect, get_relative_positions, get_snp_status, process_reads = args.output_dir, args.mutations_dir, args.rna_dir, args.results_prefix, args.subset, args.subset_no, args.clean_run, args.snp_ops, args.process_mutations, args.intersect_mutations, args.convert_intersect, args.get_relative_positions, args.get_snp_status, args.process_reads
    ...
    # if you want to process snps or full clean run
    if snp_ops or clean_run:
        ...
        if get_snp_status or clean_run or not os.path.isfile(disease_ptcs_file) or not os.path.isfile(disease_other_file):
