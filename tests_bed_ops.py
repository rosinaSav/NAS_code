...
class Test_bed_ops(unittest.TestCase):
    ...
    def test_extract_cds_quality_control(self):
        gtf_file = "./test_data/test_extract_cds_quality_control.gtf"
        genome_file = "./test_data/test_extract_cds_quality_control_genome.fa"
        observed = "./test_data/test_extract_cds_quality_control_fasta_observed.fasta"
        gen.remove_file(observed)
        expected = gen.read_fasta("./test_data/test_extract_cds_quality_control_fasta_expected.fasta")
    ...
    def test_extract_cds_from_bed_quality_control(self):
        bed_file = "./test_data/test_extract_cds_from_bed_quality_control_bed.bed"
        observed = "./test_data/test_extract_cds_from_bed_quality_control_fasta_observed.fasta"
        gen.remove_file(observed)
        expected = gen.read_fasta("./test_data/test_extract_cds_from_bed_quality_control_fasta_expected.fasta")
        extract_cds_from_bed(bed_file, observed, "./test_data/test_extract_cds_from_bed_quality_control_genome.fa", random_directory=True, check_acgt=True, check_start=True, check_length=True, check_stop=True, check_inframe_stop=True)
        observed = gen.read_fasta(observed)
        self.assertEqual(observed, expected)
    ...
    def test_fasta_sequence_quality_control(self):
        fasta_parts = gen.read_fasta('./test_data/test_fasta_sequence_quality_control.fasta')
